...
r=np.matrix(df[['h','l','c','t']]['2013':'2014'])
r[:,0:3]=1/r[:,0:3]
r[:,3]=(r[:,3]==False)
r[:,[0,1]]=r[:,[1,0]]

xi,yi = create_dataset(r)
...
def buildModel():
    ...
    model = Sequential()
    model.add(Convolution1D(input_shape=X.shape[1:],nb_filter=32, filter_length=5, border_mode='same', activation='relu'))  #--Raised to 5 makes difference
    ...
    model.add(Flatten())
    #model.add(Dense(16, activation='relu')) #---Not adding Much
...
def trainModel(maxEpoch,model):
    ...
    for i in range(maxEpoch):
        trn=model.fit(x, y, nb_epoch=1, batch_size=64,verbose=0)
        yP=model.predict(xt)
...
np.random.seed(1664)
model=buildModel()
trainPerf, testPerf=trainModel(5,model)
